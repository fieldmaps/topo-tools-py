...
def main(
    ...
    DEFAULT_DISTANCE: effective_distance = MAX(MIN(DEFAULT_DISTANCE,
    natural_res), total_exterior_length / target_point_budget). natural_res
    (the median real segment length) lets files with genuinely finer source
    detail than DEFAULT_DISTANCE start there instead of losing that detail to
    a coarser default; the budget term protects files whose exterior
    boundary would otherwise generate more points than --memory-gb can
    safely hold. Neither term can affect files whose segments are
    pathologically long — MAX_POINTS_PER_SEGMENT caps those independently.
    ...
    against memory_gb: this cost is independent of the resampling distance,
    so no amount of doubling it in the retry loop below can rescue a file
    whose raw vertex count alone already exceeds the budget. --memory-gb is
    a soft target, not a hard limit — the real deployment may have swap
    headroom beyond it — so this only logs a warning and falls back to
    DEFAULT_DISTANCE; it never refuses to attempt.
    ...
    if natural_res is None or total_length is None:
        ...
        if usable_mb <= 0:
            logger.warning(
                ...
                "before any resampling is applied — attempting anyway with "
                "DEFAULT_DISTANCE since --memory-gb is a soft target",
                name,
                f"{raw_segment_count:,}",
                remerge_floor_mb,
                memory_gb * 1024,
            ...
            candidate = min(float(DEFAULT_DISTANCE), natural_res)
